...
이전 강좌에서 저희는 z좌표를 잊어버린체 orthographic projection(직교투영, 평(형)투영, 정형투형)으로 모델을 렌더링 하였습니다.
오늘 목표는 perspective(원근투형, 투시투형)으로 그리기입니다.
...
평면에서의 선형 변형(linear transformation)은 메트릭스로 표현할 수 있습니다.
점(x, y)이 있으면, transformation은 다음과 같습니다:
A linear transformation on a plane can be represented by a corresponding matrix.
 If we take a point (x,y) then its transformation can be written as follows:

![](https://raw.githubusercontent.com/ssloy/tinyrenderer/gh-pages/img/04-perspective-projection/f00.png)


가장 간단한 transformation은 identity이며, 이는 어떠한 점으로도 이동할 수 없습니다.
...
메트릭스의 대각 계수(Diagonal coefficients)는 스케일링(scaling)을 담당합니다.
다음 transformation을 취했다고 한번 상상해봅시다:
 of the matrix give scaling along coordinate axes. Let us illustrate it, if we take the following transformation:

![](https://raw.githubusercontent.com/ssloy/tinyrenderer/gh-pages/img/04-perspective-projection/f02.png)


하얀색 물체(한쪽 모서리가 접힌 사각형)은 노란색으로 변형(be transformed)될 것입니다.
Then the white object (the white square with one corner chopped) will be transformed into the yellow one.
빨강과 녹색 선은 각각 x, y에 할당된 유닛 벡터입니다:
 Red and green line segments give unit length vectors aligned with x and y, respectively:
...
왜 메트릭스에 대해 계속 파고들까요? 매우 유용하기 때문입니다.
무엇보다도 메트릭스로
다음과 같이 전체 오브젝트의 transformation을 표현할 수 있습니다.
...
이 표현식에서의
transformation 메트릭스는 이전것과 같지만 2x5 메트릭스는
In this expression the transformation matrix is the same as in the previous one, but the 2x5 matrix is nothing else but the vertices of our squarish object.

배열에 있는 모든 버텍스들을 모아 transformation 메트릭스로 곱하면, transformed된 오브젝트를 얻을 수 있습니다. 쩔지 않습니까?
 We simply took all the vertices in an array, multiplied it by the transformation matrix and obtained the transformed object. Cool, is not it?
